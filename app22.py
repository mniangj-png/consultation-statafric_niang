...
# =========================
# Stockage Cloud (Google Sheets) : source de vérité en production
# =========================
# Remarque : sur Streamlit Community Cloud, le système de fichiers local peut être effacé / réinitialisé.
# Si GOOGLE_SHEET_ID + GOOGLE_SERVICE_ACCOUNT sont fournis via secrets, l'app écrit en priorité dans Google Sheets.
DEFAULT_GOOGLE_SHEET_ID = "1v6BZRszVLgW_FDWRszpb4jrGFMVUziIxB567j99PyU8"
GSHEET_WS_RESPONSES = "responses"
GSHEET_WS_DRAFTS = "drafts"

# Fallback local (debug uniquement). Mettre à True dans secrets.toml si vous souhaitez conserver SQLite en plus.
ENABLE_SQLITE_FALLBACK_DEFAULT = False

# Si True : empêcher plusieurs soumissions pour un même email (non recommandé si vous souhaitez conserver la plus récente).
...
def autosave_draft(force: bool = False) -> Tuple[bool, str]:
    """Sauvegarde automatique (brouillon) : priorité Google Sheets, fallback SQLite optionnel."""
    draft_id = st.session_state.get("draft_id")
    email = (resp_get("email", "") or "").strip().lower()
    ...
    # Anti-spam : ne sauvegarder qu'au plus toutes les 2 secondes (sauf force=True)
    ...
    enable_sqlite_fallback = bool(st.secrets.get("ENABLE_SQLITE_FALLBACK", ENABLE_SQLITE_FALLBACK_DEFAULT))

    # 1) Google Sheets (prioritaire)
    gs_ok, gs_msg = google_sheets_append_draft(draft_id, email, payload)
    if gs_ok:
        st.session_state.last_draft_save_ts = now_ts
        return True, "saved_gsheets"

    # 2) Fallback SQLite (optionnel)
    if enable_sqlite_fallback:
        try:
            db_save_draft(draft_id, email, payload)
            st.session_state.last_draft_save_ts = now_ts
            return True, "saved_sqlite_fallback"
        except Exception as e:
            return False, f"sqlite_error:{e}"

    return False, f"gsheets_error:{gs_msg}"
...
def db_load_draft(draft_id: str) -> Optional[Dict[str, Any]]:
    """Charge un brouillon par rid : priorité Google Sheets, fallback SQLite optionnel."""
    draft_id = (draft_id or "").strip()
    if not draft_id:
        return None

    # 1) Google Sheets
    gs_payload = google_sheets_get_draft_payload(draft_id)
    if isinstance(gs_payload, dict) and gs_payload:
        return gs_payload

    # 2) Fallback SQLite (optionnel)
    enable_sqlite_fallback = bool(st.secrets.get("ENABLE_SQLITE_FALLBACK", ENABLE_SQLITE_FALLBACK_DEFAULT))
    if (not enable_sqlite_fallback) or (not os.path.exists(DB_PATH)):
        return None
...
def _extract_sheet_id(value: Optional[str]) -> Optional[str]:
    """Extrait un sheet_id depuis une URL Google Sheets ou renvoie la valeur si elle ressemble déjà à un ID."""
    if not value:
        return None
    v = str(value).strip()
    if not v:
        return None
    # URL type : https://docs.google.com/spreadsheets/d/<ID>/edit...
...
def _get_gsheets_config() -> Tuple[Optional[str], Optional[Dict[str, Any]], List[str], str]:
    """
    Récupère la configuration Google Sheets depuis st.secrets.

    Supporte 2 formats :
    1) Format recommandé (gspread direct) :
        - GOOGLE_SHEET_ID = "..."
        - [GOOGLE_SERVICE_ACCOUNT] {...}
    2) Format Streamlit connection (si déjà utilisé) :
        - [connections.gsheets] spreadsheet = "https://..." + champs service_account

    Retour :
      (sheet_id, service_account_info, missing_items, source_label)
    """
    missing: List[str] = []
    source = "GOOGLE_SHEET_ID / GOOGLE_SERVICE_ACCOUNT"

    # 1) Format recommandé
    sheet_id = _extract_sheet_id(st.secrets.get("GOOGLE_SHEET_ID", DEFAULT_GOOGLE_SHEET_ID))
    sa_info = st.secrets.get("GOOGLE_SERVICE_ACCOUNT", None)

    # 2) Fallback : connections.gsheets
    if (not sheet_id or not sa_info) and "connections" in st.secrets:
        try:
            conn = st.secrets.get("connections", {})
            gs = conn.get("gsheets", None) if isinstance(conn, dict) else None
            if isinstance(gs, dict):
                source = "connections.gsheets"
                sheet_id = sheet_id or _extract_sheet_id(gs.get("spreadsheet"))
                # Retirer la clé 'spreadsheet' pour ne garder que la partie credentials
                sa_info = sa_info or {k: v for k, v in gs.items() if k != "spreadsheet"}
        except Exception:
            pass

    if gspread is None or Credentials is None:
        missing.append("dépendances : gspread / google-auth")

    if not sheet_id:
        missing.append("secret : GOOGLE_SHEET_ID (ou connections.gsheets.spreadsheet)")
    if not sa_info:
        missing.append("secret : GOOGLE_SERVICE_ACCOUNT (ou credentials dans connections.gsheets)")

    return sheet_id, sa_info, missing, source


def gsheets_is_configured() -> bool:
    """True si l'app dispose des secrets + dépendances nécessaires pour écrire dans Google Sheets."""
    sheet_id, sa_info, missing, _ = _get_gsheets_config()
    return (len(missing) == 0) and bool(sheet_id) and bool(sa_info)


def gsheets_config_message() -> str:
    """Message explicatif pour aider à corriger la configuration."""
    sheet_id, sa_info, missing, source = _get_gsheets_config()
    if not missing:
        return f"Google Sheets configuré (source secrets : {source})."
    # Message actionnable (sans exposer de secrets)
    items = "; ".join(missing)
    return (
        "Google Sheets non configuré : " + items + ".\n"
        "Vérifiez dans Streamlit Cloud : Manage app → Settings → Secrets.\n"
        "Format recommandé : GOOGLE_SHEET_ID + [GOOGLE_SERVICE_ACCOUNT]."
    )


@st.cache_resource(show_spinner=False)
def _gsheets_open():
    """Ouvre le classeur Google Sheets (mise en cache Streamlit)."""
    sheet_id, sa_info, missing, source = _get_gsheets_config()
    if missing:
        raise RuntimeError(gsheets_config_message())
    scopes = [
...
def _gsheets_get_ws(title: str, rows: int = 5000, cols: int = 120):
    sh = _gsheets_open()
    try:
...
def _gsheets_append_row_dict(ws, row: Dict[str, Any]) -> None:
    """Append d'un dict : crée/complète l'en-tête au besoin."""
...
def google_sheets_append_draft(draft_id: str, email: str, payload: Dict[str, Any]) -> Tuple[bool, str]:
    """Append d'un brouillon dans l'onglet GSHEET_WS_DRAFTS."""
    if not gsheets_is_configured():
        return False, gsheets_config_message()
    try:
        ws = _gsheets_get_ws(GSHEET_WS_DRAFTS, rows=8000, cols=20)
        row = {
            "draft_id": (draft_id or "").strip(),
        ...
        _gsheets_append_row_dict(ws, row)
        return True, "OK"
    except Exception as e:
        return False, f"Erreur Google Sheets (drafts) : {e}"

def google_sheets_get_draft_payload(draft_id: str) -> Optional[Dict[str, Any]]:
    """Récupère le dernier brouillon (rid) dans GSHEET_WS_DRAFTS."""
    if not gsheets_is_configured():
        return None
    draft_id = (draft_id or "").strip()
    if not draft_id:
    ...
    try:
        ...
        if "draft_id" not in header or "payload_json" not in header:
            return None
        i_did = header.index("draft_id")
        i_payload = header.index("payload_json")
        # Dernière occurrence (on scanne à rebours)
        for r in reversed(values[1:]):
            if len(r) <= max(i_did, i_payload):
                continue
            if (r[i_did] or "").strip() == draft_id:
                raw = r[i_payload] or ""
                if not raw:
                    return None
                try:
...
def google_sheets_append(payload: Dict[str, Any]) -> Tuple[bool, str]:
    """Append d'une soumission finale dans l'onglet GSHEET_WS_RESPONSES."""
    if not gsheets_is_configured():
        return False, gsheets_config_message()
    try:
        ws = _gsheets_get_ws(GSHEET_WS_RESPONSES, rows=20000, cols=200)
        row = flatten_payload(payload)
        _gsheets_append_row_dict(ws, row)
        return True, "OK"
    except Exception as e:
        return False, f"Erreur Google Sheets (responses) : {e}"
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    # - Par défaut : on bloque seulement les doubles clics dans la même session.
    # - Option (secrets) : ENFORCE_UNIQUE_EMAIL=True pour interdire plusieurs soumissions par email (non recommandé si vous souhaitez conserver la plus récente).
    email = (resp_get("email", "") or "").strip().lower()
    enforce_unique_email = bool(st.secrets.get("ENFORCE_UNIQUE_EMAIL", ENFORCE_UNIQUE_EMAIL_DEFAULT))
    enable_sqlite_fallback = bool(st.secrets.get("ENABLE_SQLITE_FALLBACK", ENABLE_SQLITE_FALLBACK_DEFAULT))

    already_in_db = False
    if enforce_unique_email and enable_sqlite_fallback and email:
        try:
    ...
    if enforce_unique_email and already_in_db:
        st.error(
            t(
                lang,
    ...
    disable_submit = (enforce_unique_email and already_in_db) or already_in_session
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        payload["draft_id"] = st.session_state.get("draft_id")

        # Stockage : Google Sheets en priorité (source de vérité). SQLite en fallback optionnel.
        ...
        # Si aucun stockage n'a réussi, ne pas valider l'envoi
        if (not gs_ok) and (not sqlite_ok):
            st.error(
                t(
                    lang,
                    "Échec d'enregistrement. Merci de réessayer ou de contacter l'équipe technique.",
                    "Save failed. Please retry or contact the technical team.",
                )
            )
            st.caption(f"Google Sheets : {gs_msg}")
            if enable_sqlite_fallback and sqlite_msg:
                st.caption(f"SQLite : {sqlite_msg}")
            return
        ...
        if gs_ok:
            ...
            st.warning(
                t(
                    lang,
                    "Votre questionnaire a été enregistré localement (mode secours).",
                    "Your submission was saved locally (fallback mode).",
                )
            )
        ...
        if (not gs_ok) and gs_msg:
            st.caption(f"Google Sheets : {gs_msg}")
        if enable_sqlite_fallback and (not sqlite_ok) and sqlite_msg:
            st.caption(f"SQLite : {sqlite_msg}")
        if (not dbx_ok) and dbx_msg:
            # Dropbox est optionnel : informer sans bloquer
            st.caption(f"Dropbox : {dbx_msg}")
