...
st.title("Consultation : Statistiques Prioritaires")
st.markdown("""
**Contexte :** Cette consultation vise à identifier les domaines statistiques prioritaires 
pour l'Agenda d'Intégration de l'Union Africaine (SHaSA 2).
Vos réponses seront traitées de manière confidentielle par STATAFRIC.
""")
...
with col2:
    ...
    fonction = st.text_input("Fonction / Titre")
    # Liste simplifiée pour l'exemple
    pays = st.selectbox("Pays / Zone", ["Sénégal", "Côte d'Ivoire", "Bénin", "Burkina Faso", "Siège UA", "Autre"])

# --- 2. SÉLECTION DES DOMAINES (Longue Liste) ---
st.header("2. Sélection des Domaines")
st.info("Veuillez sélectionner au moins 5 domaines pertinents dans la liste ci-dessous.")

# Liste des domaines (Basée sur votre app22 précédente)
domaines_shasa = [
    "1.1 Comptes nationaux", "1.2 Statistiques des prix", "1.3 Finances publiques",
    "1.4 Monnaie et banques", "1.5 Commerce extérieur (Biens & Services)",
    "1.6 Balance des paiements", "2.1 Population et migration", "2.2 Travail et emploi",
    "2.3 Santé et nutrition", "2.4 Éducation", "2.5 Pauvreté et inégalités",
    "2.6 Culture et tourisme", "3.1 Agriculture et pêche", "3.2 Environnement et climat",
    "3.3 Énergie et mines", "3.4 Eau et assainissement", "3.5 Infrastructures et transport",
    "3.6 TIC et économie numérique", "4.1 Gouvernance politique", "4.2 Droits de l'homme",
    "4.3 Paix et sécurité"
]

selection_q1 = st.multiselect(
    "Quels sont les domaines statistiques prioritaires pour l'intégration ?",
    options=domaines_shasa
)

nb_selection = len(selection_q1)
if nb_selection < 5:
    st.warning(f"Veuillez sélectionner encore au moins {5 - nb_selection} domaines.")
else:
    st.success(f"{nb_selection} domaines sélectionnés.")

    # --- 3. HIÉRARCHISATION (TOP 5) ---
    st.header("3. Hiérarchisation (TOP 5)")
    st.markdown("Parmi votre sélection, veuillez classer les 5 plus urgents.")

    # Logique dynamique : On retire les choix précédents des listes suivantes
    top1 = st.selectbox("Priorité N°1 (La plus urgente)", options=[""] + selection_q1)

    rest_2 = [x for x in selection_q1 if x != top1]
    top2 = st.selectbox("Priorité N°2", options=[""] + rest_2, disabled=(top1 == ""))

    rest_3 = [x for x in rest_2 if x != top2]
    top3 = st.selectbox("Priorité N°3", options=[""] + rest_3, disabled=(top2 == ""))

    rest_4 = [x for x in rest_3 if x != top3]
    top4 = st.selectbox("Priorité N°4", options=[""] + rest_4, disabled=(top3 == ""))

    rest_5 = [x for x in rest_4 if x != top4]
    top5 = st.selectbox("Priorité N°5", options=[""] + rest_5, disabled=(top4 == ""))

    # --- 4. JUSTIFICATION ---
    st.header("4. Justification")
    justification = st.text_area("Pourquoi ce choix de priorités ? (Contexte national ou régional)", height=100)

    # --- SOUMISSION ---
    if st.button("Soumettre mes réponses", type="primary"):
        # Vérification finale
        if top1 and top2 and top3 and top4 and top5 and nom and email:

            with st.spinner('Enregistrement sécurisé en cours sur Google Sheets...'):
                try:
                    ...
                    # 2. Lecture des données existantes (pour ne pas écraser)
                    # On utilise l'URL de votre Sheet définie dans les secrets
                    data_existante = conn.read(worksheet="Feuille 1", ttl=0)

                    # 3. Concaténation (Ajout à la suite)
                    data_updated = pd.concat([data_existante, nouvelle_donnee], ignore_index=True)

                    # 4. Envoi vers le Cloud
                    conn.update(data=data_updated)

                    st.balloons()
                    st.success("✅ Vos réponses ont été bien enregistrées par STATAFRIC. Merci de votre contribution !")

                except Exception as e:
                    st.error(f"Une erreur technique est survenue : {e}")
        else:
            st.error("Veuillez remplir votre Nom, Email et définir intégralement le TOP 5 avant de soumettre.")
import streamlit as st
import pandas as pd
from datetime import datetime
from streamlit_gsheets import GSheetsConnection
