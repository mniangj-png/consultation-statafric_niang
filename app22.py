...
def gsheets_is_configured() -> bool:
    """True si l'app dispose des secrets nécessaires pour écrire dans Google Sheets."""
    ...
    return bool(sa_info) and bool(sheet_id)
...
def _gsheets_open():
    ...
    sa_info = st.secrets.get("GOOGLE_SERVICE_ACCOUNT", None)
    if not sa_info:
        raise RuntimeError("GOOGLE_SERVICE_ACCOUNT manquant dans secrets.")
    scopes = [
...
def _gsheets_append_row_dict(ws, row: Dict[str, Any]) -> None:
    ...
    # Ordre des valeurs selon l'en-tête
    header = ws.row_values(1)
...
def google_sheets_append_draft(draft_id: str, email: str, payload: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if not gsheets_is_configured():
        return False, "Google Sheets non configuré (secrets manquants ou dépendances absentes)."
    try:
