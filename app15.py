...
def admin_login(lang: str) -> None:
    ...
    if not expected:
        env_pw = os.getenv("ADMIN_PASSWORD", None)
        if env_pw:
            expected = env_pw
            source = "env"
    ...
    if source is None:
        st.caption(
            t(
                lang,
                "ADMIN_PASSWORD : non configuré (secrets ou variable d’environnement).",
                "ADMIN_PASSWORD : not configured (secrets or environment variable).",
            )
        )
    else:
        st.caption(
            t(
                lang,
                f"ADMIN_PASSWORD : configuré via {source}.",
                f"ADMIN_PASSWORD : configured via {source}.",
            )
        )
    ...
    if st.button(t(lang, "Se connecter", "Login")):
        if expected and pw == expected:
            ...
            # Message conservé (identique à l’application initiale), pour éviter toute confusion côté utilisateur.
            st.error(
                t(
                    lang,
