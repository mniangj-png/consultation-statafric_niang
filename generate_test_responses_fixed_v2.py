...
def generate_payload(
    ...
    # The app asks for a TOP 5, but a malformed/partial longlist could contain < 5 domains.
    # In that case, degrade gracefully instead of crashing.
    k_top5 = min(5, len(preselected_domains))
    top5_domains = random.sample(preselected_domains, k=k_top5)
...
def main() -> None:
    ...
    # Informations utiles (visible dans les logs Streamlit Cloud)
    total_stats = sum(len(v) for v in stats_by_domain.values())
    print(f'[INFO] Longlist utilisée : {longlist_path} | domaines : {len(domains)} | indicateurs : {total_stats}')
    print(f'[INFO] Fichier pays utilisé : {countries_path} | pays : {len(countries)}')
    if len(domains) < 5:
        print(f'[AVERTISSEMENT] La longlist ne contient que {len(domains)} domaines : la sélection TOP5 sera limitée.')
