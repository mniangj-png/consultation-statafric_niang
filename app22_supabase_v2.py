...
def db_save_submission(submission_id: str, lang: str, email: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - Postgres : payload_json (jsonb)
    - Storage (optionnel) : un fichier JSON par soumission
    """
    if not _sb_enabled():
        raise RuntimeError("Supabase non configuré (SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).")
    ...
    info: Dict[str, Any] = {"db_ok": True, "storage_enabled": bool(SUPABASE_STORAGE_BUCKET)}
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        # Enregistrement Supabase (Postgres jsonb) + fichier JSON (Storage optionnel)
        try:
        ...
        try:
            if isinstance(sb_info, dict) and sb_info.get("storage_enabled"):
                if sb_info.get("storage_ok"):
                    st.caption(t(lang, "JSON : fichier enregistré dans Supabase Storage.", "JSON: file saved in Supabase Storage."))
                else:
                    st.warning(t(lang, "JSON : fichier non enregistré dans Storage (mais la base Postgres est enregistrée).",
                                 "JSON: file not saved in Storage (but Postgres is saved)."))
        except Exception:
            pass
