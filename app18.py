...
def flatten_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Rubrique 8 : priorité genre principale
    out["gender_main_priority"] = payload.get("gender_main_priority", "")
    out["gender_main_priority_other"] = payload.get("gender_main_priority_other", "")
...
def validate_r8(lang: str) -> List[str]:
    ...
    # Gender main priority : mandatory (1 choice). If 'Other' then specify
    g = (resp_get("gender_main_priority", "") or "").strip()
    if not g:
        errs.append(
            t(
                lang,
                "Rubrique 8 : veuillez sélectionner votre priorité genre principale.",
                "Section 8: please select your main gender priority."
            )
        )
    elif g in ("Autre", "Other"):
        other = (resp_get("gender_main_priority_other", "") or "").strip()
        if not other:
            errs.append(
                t(
                    lang,
                    "Rubrique 8 : précisez l’option « Autre » pour la priorité genre.",
                    "Section 8: please specify the “Other” option for the gender priority."
                )
            )

    # Capacity table (mandatory)
    tbl = resp_get("capacity_table", {})
    if not isinstance(tbl, dict) or not tbl:
        errs.append(
            t(
                lang,
                "Rubrique 8 : veuillez renseigner le tableau « Capacité et faisabilité ».",
                "Section 8: please complete the “Capacity and feasibility” table."
            )
        )
        return errs
    ...
    for k, v in tbl.items():
        if not v:
            errs.append(
                t(
                    lang,
                    f"Rubrique 8 : ligne non renseignée : {k}.",
                    f"Section 8: missing answer for: {k}."
                )
            )
    return errs
...
def get_steps(lang: str) -> List[Tuple[str, str]]:
    ...
    return [
        ...
        ("R8", t(lang, "Rubrique 8 : Dimension genre", "Section 8: Gender dimension")),
...
### Comment répondre
N'hésitez pas de consulter les infobulles (? encerclé) pour plus de précisions.
...
### How to answer
Feel free to consult the tooltips (circled ?) for more details.
...
def rubric_8(lang: str) -> None:
    st.subheader(t(lang, "Rubrique 8 : Dimension genre", "Section 8: Gender dimension"))

    # Main gender priority (single choice)
    st.markdown(
        t(
            lang,
            "Merci d’indiquer votre **priorité genre principale**. Si vous choisissez « Autre », veuillez préciser.",
            "Please indicate your **main gender priority**. If you select “Other”, please specify."
        )
    )
    ...
    prev = (resp_get("gender_main_priority", "") or "").strip()
    all_opts = [""] + opts
    placeholder = t(lang, "— Sélectionner —", "— Select —")

    chosen = st.selectbox(
        t(lang, "Votre priorité genre principale", "Your main gender priority"),
        options=all_opts,
        index=all_opts.index(prev) if prev in all_opts else 0,
        format_func=lambda x: placeholder if x == "" else x,
        key="gender_main_priority_select",
        help=t(
            lang,
            "Sélectionnez une priorité principale. Choisissez « Autre » si aucune option ne correspond.",
            "Select one main priority. Choose “Other” if none of the options fit."
        ),
    )
    resp_set("gender_main_priority", chosen)

    if chosen in ("Autre", "Other"):
        other_txt = st.text_input(
            t(lang, "Autre : préciser", "Other: specify"),
            key="gender_main_priority_other_input",
            value=resp_get("gender_main_priority_other", ""),
        )
        resp_set("gender_main_priority_other", (other_txt or "").strip())
    else:
        resp_set("gender_main_priority_other", "")

    st.markdown("---")
    st.markdown("### " + t(lang, "Capacité et faisabilité (12-24 mois)", "Capacity and feasibility (12-24 months)"))
    st.markdown(
        t(
            lang,
