...
def rubric_9(lang: str) -> None:
    ...
    st.markdown(
        t(
            lang,
            "Indiquez les exigences clés attendues (commencez par la plus importante) en matière d’harmonisation et d’assurance qualité.",
            "Indicate key expectations (start with the most important one) regarding harmonization and quality assurance."
        )
    )
...
def rubric_10(lang: str) -> None:
    ...
    st.markdown(
        t(
            lang,
            "Indiquez les canaux de diffusion jugés les plus utiles (commencez par le plus utile) pour les statistiques prioritaires.",
            "Indicate the dissemination channels you find most useful (start with the most useful one) for priority statistics."
        )
    )
    opts_fr = [
