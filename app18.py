...
def validate_r8(lang: str) -> List[str]:
    ...
    # Dimension genre (priorité principale)
    gp = (resp_get("gender_priority_main", "") or "").strip()
    if not gp:
        errs.append(
            t(
                lang,
        ...
        if not other:
            errs.append(
                t(
                    ...
                    "Section 8: please specify the 'Other' option for gender priority.",
                )
            )
...
def get_steps(lang: str) -> List[Tuple[str, str]]:
    # Rubric 7 removed, plus final SEND tab
    return [
...
def rubric_8(lang: str) -> None:
    ...
    # Dimension genre : priorité principale (obligatoire)
    st.markdown("#### " + t(lang, "Votre priorité genre principale", "Your main gender priority"))

    gp_opts = ["", "ECO", "SERV", "GBV", "PART", "CARE", "OTHER"]
    gp_labels = {
        ...
        "PART": t(lang, "Participation", "Participation"),
        "CARE": t(lang, "Temps domestique non rémunéré", "Unpaid care"),
        "OTHER": t(lang, "Autre", "Other"),
    }

    prev_gp = (resp_get("gender_priority_main", "") or "").strip()
    idx_gp = gp_opts.index(prev_gp) if prev_gp in gp_opts else 0
    chosen_gp = st.selectbox(
        t(lang, "Votre priorité genre principale", "Your main gender priority"),
        options=gp_opts,
        index=idx_gp,
        format_func=lambda k: (t(lang, "— Sélectionner —", "— Select —") if k == "" else gp_labels.get(k, k)),
        key="gender_priority_main_select",
    )
    resp_set("gender_priority_main", chosen_gp)
...
def rubric_12(lang: str) -> None:
    ...
    st.markdown(
        t(
            lang,
            "Ces questions sont **optionnelles**. Vous pouvez les laisser vides.",
            "These questions are **optional**. You may leave them blank."
        )
    )
    ...
    chosen_cc = st.selectbox(
        ...
        help=t(
            lang,
            "Choisissez Oui ou Non. Cette question est obligatoire.",
            "Select Yes or No. This question is mandatory.",
        ),
