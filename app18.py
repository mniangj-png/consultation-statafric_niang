...
def validate_r7(lang: str) -> List[str]:
    ...
    r1 = (resp_get("gender_priorities_rank1", "") or "").strip()
    r2 = (resp_get("gender_priorities_rank2", "") or "").strip()
    r3 = (resp_get("gender_priorities_rank3", "") or "").strip()

    if not r1:
        errs.append(
            t(
                lang,
                "Rubrique 7 : veuillez sélectionner au moins une priorité genre (rang 1).",
                "Section 7: please select at least one gender priority (rank 1).",
            )
        )
        return errs
    ...
    if r3 and not r2:
        errs.append(
            t(
                lang,
                "Rubrique 7 : veuillez renseigner le rang 2 avant le rang 3.",
                "Section 7: please fill rank 2 before rank 3.",
            )
        )

    chosen = [x for x in [r1, r2, r3] if x]
    if len(set(chosen)) != len(chosen):
        errs.append(
            t(
                lang,
                "Rubrique 7 : une même priorité ne peut pas être sélectionnée plusieurs fois.",
                "Section 7: the same priority cannot be selected more than once.",
            )
        )

    if "OTHER" in chosen:
        other = (resp_get("gender_priorities_other", "") or "").strip()
        if not other:
            errs.append(
                t(
                    lang,
                    "Rubrique 7 : précisez l’option « Autre » pour les priorités genre.",
                    "Section 7: please specify the 'Other' option for gender priorities.",
                )
            )
...
def get_steps(lang: str) -> List[Tuple[str, str]]:
    # Rubric 7 enabled, plus final SEND tab
    return [
...
def rubric_7(lang: str) -> None:
    ...
    st.markdown(
        t(
            lang,
            "Sélectionnez de 1 à 3 priorités genre, en commençant par la plus importante (rang 1).",
            "Select 1 to 3 gender priorities, starting with the most important one (rank 1)."
        )
    )

    opts = ["", "ECO", "SERV", "GBV", "PART", "CARE", "OTHER"]
    labels = {
        ...
        "PART": t(lang, "Participation aux instances décisionnelles", "Participation in decision-making bodies"),
        "CARE": t(lang, "Temps domestique non rémunéré", "Unpaid domestic work"),
        "OTHER": t(lang, "Autre", "Other"),
    }

    # Pré-remplissage (compatibilité brouillons anciens)
    default_r1 = (resp_get("gender_priorities_rank1", "") or "").strip()
    if not default_r1:
        default_r1 = (resp_get("gender_priority_main", "") or "").strip()
    default_r2 = (resp_get("gender_priorities_rank2", "") or "").strip()
    default_r3 = (resp_get("gender_priorities_rank3", "") or "").strip()
    default_other = (resp_get("gender_priorities_other", "") or "").strip()
    if not default_other:
        default_other = (resp_get("gender_priority_other", "") or "").strip()

    def _fmt(k: str) -> str:
        return t(lang, "— Sélectionner —", "— Select —") if k == "" else labels.get(k, k)

    # Rang 1 (obligatoire)
    idx1 = opts.index(default_r1) if default_r1 in opts else 0
    r1 = st.selectbox(
        t(lang, "Rang 1 (le plus important)", "Rank 1 (most important)"),
        options=opts,
        index=idx1,
        format_func=_fmt,
        key="gender_priorities_rank1_select",
    )
    resp_set("gender_priorities_rank1", r1)

    # Rang 2 (optionnel)
    idx2 = opts.index(default_r2) if default_r2 in opts else 0
    r2 = st.selectbox(
        t(lang, "Rang 2 (optionnel)", "Rank 2 (optional)"),
        options=opts,
        index=idx2,
        format_func=_fmt,
        key="gender_priorities_rank2_select",
    )
    resp_set("gender_priorities_rank2", r2)

    # Rang 3 (optionnel)
    idx3 = opts.index(default_r3) if default_r3 in opts else 0
    r3 = st.selectbox(
        t(lang, "Rang 3 (optionnel)", "Rank 3 (optional)"),
        options=opts,
        index=idx3,
        format_func=_fmt,
        key="gender_priorities_rank3_select",
    )
    resp_set("gender_priorities_rank3", r3)

    chosen = [x for x in [r1, r2, r3] if x]

    # Autre : préciser si sélectionné (dans n'importe quel rang)
    if "OTHER" in chosen:
        other = st.text_input(
            t(lang, "Autre : préciser", "Other: please specify"),
            key="gender_priorities_other_input",
            value=default_other,
        )
        resp_set("gender_priorities_other", (other or "").strip())
    else:
        resp_set("gender_priorities_other", "")

    # Stockage consolidé (utile pour exports)
    resp_set("gender_priorities", "|".join(chosen))
...
def rubric_9(lang: str) -> None:
    ...
    sel = st.multiselect(t(lang, "Sélectionnez", "Select"), options=opts, key="r9_multiselect")
