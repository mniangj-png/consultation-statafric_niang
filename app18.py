...
def validate_r9(lang: str) -> List[str]:
    ...
    clean_sel = [x for x in (sel if isinstance(sel, list) else []) if str(x).strip() != ""]
    ...
    if len(clean_sel) > 3:
        errs.append(t(lang, "Rubrique 9 : sélectionnez au maximum 3 options.", "Section 9: select at most 3 options."))

    has_other = any(
        str(x).strip().lower().startswith("autre") or str(x).strip().lower().startswith("other")
        for x in clean_sel
    )
    if has_other:
...
def validate_r10(lang: str) -> List[str]:
    ...
    if len(clean_sel) > 3:
        errs.append(t(lang, "Rubrique 10 : sélectionnez au maximum 3 options.", "Section 10: select at most 3 options."))
...
def rubric_9(lang: str) -> None:
    ...
    # Limite : 1 à 3 choix (max_selections si disponible)
    try:
        sel = st.multiselect(
            t(lang, "Sélectionnez (1 à 3)", "Select (1 to 3)"),
            options=opts,
            key="r9_multiselect",
            max_selections=3,
        )
    ...
    if has_other:
        st.text_input(
            t(lang, "Préciser (Autre)", "Specify (Other)"),
            key="q9_other_input",
            value=resp_get("quality_other", ""),
        )
...
def rubric_10(lang: str) -> None:
    ...
    if has_other:
        st.text_input(
            t(lang, "Préciser (Autre)", "Specify (Other)"),
            key="q10_other_input",
            value=resp_get("dissemination_other", ""),
        )
...
# --- Superadmin : génération du classeur d'analyse (Excel) ---
ANALYSIS_TEMPLATE_DEFAULT_XLSX = "Classeur_analyse_consultation_STATAFRIC_final.xlsx"

def find_analysis_template_path() -> Optional[str]:
    """Localise le classeur modèle (template) utilisé pour générer le classeur d'analyse."""
    candidates: List[str] = []
    env_p = os.environ.get("ANALYSIS_TEMPLATE_XLSX")
    if env_p:
        candidates.append(env_p)
    candidates += [
        os.path.join("data", ANALYSIS_TEMPLATE_DEFAULT_XLSX),
        ANALYSIS_TEMPLATE_DEFAULT_XLSX,
    ]
    for p in candidates:
        try:
...
def _score_int_to_level(v: Any) -> str:
    """Convertit une note 0-3 en libellé {High, Med, Low, UK}."""
    try:
        iv = int(v)
    except Exception:
        return "UK"
...
def _cap_code_to_level(v: Any) -> str:
    if v is None:
        return "UK"
    s = str(v).strip().upper()
    return {"HIGH": "High", "MED": "Med", "LOW": "Low", "UK": "UK"}.get(s, "UK")

def _gender_code_to_oui_non(v: Any) -> str:
    if v is None:
        return "UK"
    s = str(v).strip().upper()
    if s == "YES":
...
def _copy_row_style(ws: "openpyxl.worksheet.worksheet.Worksheet", src_row: int, dst_row: int, max_col: Optional[int] = None) -> None:
...
def _ensure_rows(ws: "openpyxl.worksheet.worksheet.Worksheet", required_last_row: int, template_row: int = 2) -> None:
    """Garantit que la feuille a au moins required_last_row lignes (en copiant les styles de template_row)."""
    if required_last_row <= ws.max_row:
        return
    old_max = ws.max_row
    add_n = required_last_row - old_max
    ws.insert_rows(old_max + 1, add_n)
    for r in range(old_max + 1, required_last_row + 1):
        _copy_row_style(ws, template_row, r)

def _clear_cells(ws: "openpyxl.worksheet.worksheet.Worksheet", start_row: int, end_row: int, start_col: int, end_col: int) -> None:
...
def _write_rows(ws: "openpyxl.worksheet.worksheet.Worksheet", start_row: int, start_col: int, rows: List[List[Any]]) -> None:
    for i, row in enumerate(rows):
        r = start_row + i
        for j, val in enumerate(row):
            ws.cell(r, start_col + j).value = val
...
def build_analysis_workbook_bytes(
    ...
    """
    Génère un classeur d'analyse à partir du modèle Excel et des réponses.

    Retourne :
      - bytes du fichier .xlsx
      - nom de fichier proposé
      - message info (pour affichage UI)
    """
    tpl = find_analysis_template_path()
    if not tpl:
        raise FileNotFoundError(
            "Classeur modèle introuvable. Ajoutez le fichier "
            f"'{ANALYSIS_TEMPLATE_DEFAULT_XLSX}' à la racine ou au dossier 'data/', "
            "ou définissez la variable d’environnement ANALYSIS_TEMPLATE_XLSX."
        )

    # Longlist (domaine / indicateur) pour libellés
    if df_long is None:
        df_long = load_longlist()

    # Index longlist
    long_by_code: Dict[str, Dict[str, Any]] = {}
    try:
        for _, r in df_long.iterrows():
            long_by_code[str(r.get("stat_code", "")).strip()] = {
                "domain_code": str(r.get("domain_code", "")).strip(),
                "domain_label_fr": str(r.get("domain_label_fr", "")).strip(),
                "domain_label_en": str(r.get("domain_label_en", "")).strip(),
                "stat_label_fr": str(r.get("stat_label_fr", "")).strip(),
                "stat_label_en": str(r.get("stat_label_en", "")).strip(),
            }
    except Exception:
        long_by_code = {}

    # Ordonner les répondants
    df0 = submissions_df.copy()
    if "submitted_at_utc" in df0.columns:
        df0["submitted_at_utc"] = pd.to_datetime(df0["submitted_at_utc"], errors="coerce", utc=True)
        df0 = df0.sort_values("submitted_at_utc", ascending=True)
    else:
        df0 = df0.reset_index(drop=True)

    # Construire un mapping submission_id -> Rxxx (stable sur l’ordre)
    sid_order: List[str] = df0.get("submission_id", pd.Series(dtype=str)).astype(str).tolist()
    sid_to_r: Dict[str, str] = {sid: f"R{i:03d}" for i, sid in enumerate(sid_order, start=1)}

    # Préparer les tables à écrire
    respondents_rows: List[List[Any]] = []
    domains_rows: List[List[Any]] = []
    indicators_rows: List[List[Any]] = []
    scoring_rows: List[List[Any]] = []
    gender_rows: List[List[Any]] = []
    capacity_rows: List[List[Any]] = []
    quality_rows: List[List[Any]] = []
    diffusion_rows: List[List[Any]] = []

    # Domaines de référence
    domains_all: List[str] = sorted([d for d in df_long.get("domain_code", pd.Series(dtype=str)).dropna().unique().tolist() if str(d).strip() != ""])

    # Helpers (alias FR/EN) pour Rubrique 6/8/9/10
    gender_alias = {
        "sex": ["Désagrégation par sexe", "Disaggregation by sex"],
        "age": ["Désagrégation par âge", "Disaggregation by age"],
        "urb_rural": ["Milieu urbain / rural", "Urban / rural"],
        "disability": ["Handicap", "Disability"],
        "wealth": ["Quintile de richesse", "Wealth quintile"],
    }
    cap_alias = {
        "skills": ["Compétences statistiques disponibles", "Available statistical skills"],
        "admin": ["Accès aux données administratives", "Access to administrative data"],
        "funding": ["Financement disponible", "Available funding"],
        "it": ["Outils numériques (collecte, traitement, diffusion)", "Digital tools (collection, processing, dissemination)"],
        "legal": ["Cadre juridique pour le partage de données", "Legal framework for data sharing"],
        "coord": ["Coordination interinstitutionnelle", "Inter-institutional coordination"],
    }
    def _get_from_tbl(tbl: Any, aliases: List[str]) -> Any:
        if not isinstance(tbl, dict):
            return None
        for a in aliases:
            if a in tbl:
    ...
    # Parcours payloads dans l'ordre df0 (sid_order)
    payload_by_sid: Dict[str, Dict[str, Any]] = {}
    for p in payloads:
        sid = str(p.get("_sid") or p.get("submission_id") or p.get("sid") or "")
        if sid:
            payload_by_sid[sid] = p

    for sid in sid_order:
        p = payload_by_sid.get(sid, {})
        rid = sid_to_r.get(sid, sid)

        pays = str(p.get("pays", "")).strip()
        type_acteur = str(p.get("type_acteur", "")).strip()
        lang = str(p.get("lang", "")).strip().lower()
        scope = str(p.get("scope", "")).strip()

        # Réponses_Répondants : on remplit A-H ; I-J sont des formules (copiées/complétées plus bas)
        respondents_rows.append([
            rid,
            pays,
            "",  # Région (optionnel)
            type_acteur,
            ("English" if lang == "en" else "Français"),
            scope,
            "",  # Consulté collègues (non collecté dans cette version)
            "",  # SNDS active (non collecté dans cette version)
        ])

        # Réponses_Domaines : une ligne par (répondant x domaine)
        pre = p.get("preselection_domains", [])
        top5 = p.get("top5_domains", [])
        if not isinstance(pre, list):
            pre = []
        if not isinstance(top5, list):
            top5 = []
        pre_set = set([str(x).strip() for x in pre if str(x).strip() != ""])
        top5_list = [str(x).strip() for x in top5 if str(x).strip() != ""]
        top5_rank = {d: i + 1 for i, d in enumerate(top5_list)}

        for d in domains_all:
            sel = 1 if (d in pre_set or d in top5_rank) else 0
            rk = top5_rank.get(d, "")
            domains_rows.append([rid, d, sel, rk])

        # Indicateurs sélectionnés + scoring
        ...
        for d, lst in selected_by_domain.items():
            if isinstance(lst, list):
                for s in lst:
                    sc = str(s).strip()
                    if sc:
                        selected_stats.append(sc)

        # dédoublonner en conservant ordre
        seen = set()
        selected_stats = [x for x in selected_stats if not (x in seen or seen.add(x))]

        scoring = p.get("scoring", {})
        if not isinstance(scoring, dict):
            scoring = {}

        for scode in selected_stats:
            meta = long_by_code.get(scode, {})
            dcode = str(meta.get("domain_code", "")).strip() or scode[:3]
            label_fr = str(meta.get("stat_label_fr", "")).strip() or scode
            # Réponses_Indicateurs
            indicators_rows.append([rid, pays, dcode, scode, label_fr, "", ""])

            # Réponses_Scoring (3 critères) ; autres = UK
            sc = scoring.get(scode, {})
            if not isinstance(sc, dict):
                sc = {}
            demand = _score_int_to_level(sc.get("demand", None))
            availability = _score_int_to_level(sc.get("availability", None))
            feasibility = _score_int_to_level(sc.get("feasibility", None))
            scoring_rows.append([rid, scode, demand, "UK", availability, feasibility, "UK", "UK", "UK"])

        # Genre (Rubrique 6)
        gtbl = p.get("gender_table", {})
        g_sex = _gender_code_to_oui_non(_get_from_tbl(gtbl, gender_alias["sex"]))
        g_age = _gender_code_to_oui_non(_get_from_tbl(gtbl, gender_alias["age"]))
        g_ur = _gender_code_to_oui_non(_get_from_tbl(gtbl, gender_alias["urb_rural"]))
        g_dis = _gender_code_to_oui_non(_get_from_tbl(gtbl, gender_alias["disability"]))
        g_w = _gender_code_to_oui_non(_get_from_tbl(gtbl, gender_alias["wealth"]))
        # Priorité genre : si l'un des items est SPEC -> "Selon indicateur", sinon si au moins un Oui -> "Oui", sinon si tout Non -> "Non", sinon UK.
        raw_codes = [
            _get_from_tbl(gtbl, gender_alias["sex"]),
            _get_from_tbl(gtbl, gender_alias["age"]),
            _get_from_tbl(gtbl, gender_alias["urb_rural"]),
            _get_from_tbl(gtbl, gender_alias["disability"]),
            _get_from_tbl(gtbl, gender_alias["wealth"]),
        ]
        raw_codes_norm = [str(x).strip().upper() for x in raw_codes if x is not None]
        if "SPEC" in raw_codes_norm:
            g_prio = "Selon indicateur"
        elif any(x == "Oui" for x in [g_sex, g_age, g_ur, g_dis, g_w]):
            g_prio = "Oui"
        elif raw_codes_norm and all(x == "NO" for x in raw_codes_norm):
            g_prio = "Non"
        else:
            g_prio = "UK"
        gender_rows.append([rid, g_sex, g_age, g_ur, g_dis, g_w, "UK", "UK", g_prio])

        # Capacité (Rubrique 8)
        ctbl = p.get("capacity_table", {})
        c_sk = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["skills"]))
        c_ad = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["admin"]))
        c_fu = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["funding"]))
        c_it = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["it"]))
        c_le = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["legal"]))
        c_co = _cap_code_to_level(_get_from_tbl(ctbl, cap_alias["coord"]))
        capacity_rows.append([rid, c_sk, c_ad, c_fu, c_it, c_le, c_co])

        # Rubrique 9 (Harmonisation et qualité) : multi-sélection 1-3
        qsel = p.get("quality_expectations", [])
        if not isinstance(qsel, list):
            qsel = []
        qset = set([str(x).strip().lower() for x in qsel if str(x).strip() != ""])
        def _q_has(fr: str, en: str) -> bool:
            return fr.lower() in qset or en.lower() in qset

        q_normes = "Oui" if _q_has("Guides de normes et méthodes harmonisés (concepts, définitions, classifications).",
                                  "Harmonized statistical standards and methods (concepts, definitions, classifications).") else "Non"
        q_metadata = "Oui" if _q_has("Cohérence des définitions et comparabilité entre secteurs (administratif, enquête, registres).",
                                    "Consistency and comparability across sectors (admin, surveys, registers).") else "Non"
        q_qa = "Oui" if _q_has("Cadres d’assurance qualité (contrôles, documentation, audits qualité).",
                               "Quality assurance frameworks (checks, documentation, quality audits).") else "Non"
        q_audits = "Oui" if _q_has("Procédures de validation / certification des statistiques (revues par pairs, comités).",
                                  "Validation / certification procedures (peer reviews, committees).") else "Non"
        q_calendar = "Non"  # non collecté explicitement dans cette version
        q_comm = "Oui" if _q_has("Groupes de travail sectoriels / communautés de pratique pour l’harmonisation.",
                                 "Sector working groups / communities of practice for harmonization.") else "Non"
        quality_rows.append([rid, q_normes, q_metadata, q_qa, q_audits, q_calendar, q_comm])

        # Rubrique 10 (Diffusion) : multi-sélection 1-3
        dsel = p.get("dissemination_channels", [])
        if not isinstance(dsel, list):
            dsel = []
        dset = set([str(x).strip().lower() for x in dsel if str(x).strip() != ""])
        def _d_has(fr: str, en: str) -> bool:
            return fr.lower() in dset or en.lower() in dset

        d_dash = "Oui" if _d_has("Portail web / tableau de bord / rapport (diffusion régulière).",
                                "Web portal / dashboards / reports (regular dissemination).") else "Non"
        d_portal_api = "Oui" if (d_dash == "Oui" or _d_has("API / open data (accès machine à machine).",
                                                          "API / open data (machine-to-machine access).")) else "Non"
        d_notes = "Oui" if _d_has("Bulletins / communiqués / notes thématiques (produits éditoriaux).",
                                  "Bulletins / press releases / thematic briefs (editorial products).") else "Non"
        d_cal = "Non"  # non collecté explicitement dans cette version
        d_micro = "Oui" if _d_has("Microdonnées anonymisées (accès sous conditions).",
                                  "Anonymized microdata (access under conditions).") else "Non"
        diffusion_rows.append([rid, d_dash, d_portal_api, d_notes, d_cal, d_micro])

    # Charger le classeur modèle
    wb = openpyxl.load_workbook(tpl, data_only=False)

    # --- Mettre à jour les feuilles d'entrée principales ---
    def _fill_formulas_respondents(ws):
        # I : first occurrence of country ; J : country count flag
        for r in range(2, 2 + len(respondents_rows)):
            ws.cell(r, 9).value = f'=IF(COUNTIF($B$2:B{r},B{r})=1,B{r},"")'
            ws.cell(r, 10).value = f'=IF(I{r}<>"",1,0)'

    def _fill_formulas_domains(ws, total_rows: int):
        for r in range(2, 2 + total_rows):
            ws.cell(r, 5).value = f'=IF(D{r}="","",VLOOKUP(D{r},Paramètres!$A$12:$B$16,2,FALSE))'

    # 1) Réponses_Répondants
    ws_r = wb["Réponses_Répondants"]
    req_last = 1 + len(respondents_rows)
    _ensure_rows(ws_r, max(req_last, 2))
    # Effacer A-H ; conserver I-J (formules)
    _clear_cells(ws_r, 2, ws_r.max_row, 1, 8)
    _write_rows(ws_r, 2, 1, respondents_rows)
    _fill_formulas_respondents(ws_r)

    # 2) Réponses_Domaines
    ws_d = wb["Réponses_Domaines"]
    req_last = 1 + len(domains_rows)
    _ensure_rows(ws_d, max(req_last, 2))
    _clear_cells(ws_d, 2, ws_d.max_row, 1, 4)
    _write_rows(ws_d, 2, 1, domains_rows)
    _fill_formulas_domains(ws_d, len(domains_rows))

    # 3) Réponses_Indicateurs
    ws_i = wb["Réponses_Indicateurs"]
    req_last = 1 + len(indicators_rows)
    _ensure_rows(ws_i, max(req_last, 2))
    _clear_cells(ws_i, 2, ws_i.max_row, 1, 7)
    _write_rows(ws_i, 2, 1, indicators_rows)

    # 4) Réponses_Scoring
    ws_s = wb["Réponses_Scoring"]
    req_last = 1 + len(scoring_rows)
    _ensure_rows(ws_s, max(req_last, 2))
    _clear_cells(ws_s, 2, ws_s.max_row, 1, 9)
    _write_rows(ws_s, 2, 1, scoring_rows)

    # 5) Réponses_Genre
    ws_g = wb["Réponses_Genre"]
    req_last = 1 + len(gender_rows)
    _ensure_rows(ws_g, max(req_last, 2))
    _clear_cells(ws_g, 2, ws_g.max_row, 1, 9)
    _write_rows(ws_g, 2, 1, gender_rows)

    # 6) Réponses_Contraintes
    ws_c = wb["Réponses_Contraintes"]
    req_last = 1 + len(capacity_rows)
    _ensure_rows(ws_c, max(req_last, 2))
    _clear_cells(ws_c, 2, ws_c.max_row, 1, 7)
    _write_rows(ws_c, 2, 1, capacity_rows)

    # 7) Réponses_Qualité
    ws_q = wb["Réponses_Qualité"]
    req_last = 1 + len(quality_rows)
    _ensure_rows(ws_q, max(req_last, 2))
    _clear_cells(ws_q, 2, ws_q.max_row, 1, 7)
    _write_rows(ws_q, 2, 1, quality_rows)

    # 8) Réponses_Diffusion
    ws_di = wb["Réponses_Diffusion"]
    req_last = 1 + len(diffusion_rows)
    _ensure_rows(ws_di, max(req_last, 2))
    _clear_cells(ws_di, 2, ws_di.max_row, 1, 6)
    _write_rows(ws_di, 2, 1, diffusion_rows)

    # Mettre à jour Longlist_condensée + Référentiel_Domaines (utile pour analyses)
    try:
        # Longlist_condensée : A-D
        ws_l = wb["Longlist_condensée"]
        long_rows = []
        for _, r in df_long.iterrows():
            long_rows.append([
                str(r.get("stat_code", "")).strip(),
                str(r.get("domain_code", "")).strip(),
                str(r.get("stat_label_fr", "")).strip(),
                "",  # Source (non disponible dans la longlist actuelle)
            ])
        req_last = 1 + len(long_rows)
        _ensure_rows(ws_l, max(req_last, 2))
        _clear_cells(ws_l, 2, ws_l.max_row, 1, 4)
        _write_rows(ws_l, 2, 1, long_rows)

        # Référentiel_Domaines : A-C
        ws_rd = wb["Référentiel_Domaines"]
        dom_rows = []
        dom_seen = set()
        for _, r in df_long.iterrows():
            d = str(r.get("domain_code", "")).strip()
            if not d or d in dom_seen:
                continue
            dom_seen.add(d)
            dom_rows.append([d, str(r.get("domain_label_fr", "")).strip(), str(r.get("domain_label_en", "")).strip()])
        req_last = 1 + len(dom_rows)
        _ensure_rows(ws_rd, max(req_last, 2))
        _clear_cells(ws_rd, 2, ws_rd.max_row, 1, 3)
        _write_rows(ws_rd, 2, 1, dom_rows)
    except Exception:
        pass
    ...
    fname = f"Classeur_analyse_consultation_STATAFRIC_{ts}.xlsx"
    info = f"{len(respondents_rows)} répondant(s), {len(indicators_rows)} indicateur(s) sélectionné(s), {len(scoring_rows)} notation(s)."
    return out_bytes, fname, info
...
def admin_dashboard(lang: str) -> None:
    ...
    with tab_super:
        tab_s_ana, tab_s_gen, tab_s_dl = st.tabs([t(lang, "Analyse (tableaux)", "Dashboards"), t(lang, "Générer le classeur d'analyse", "Generate analysis workbook"), t(lang, "Télécharger le classeur d'analyse", "Download analysis workbook")])
        with tab_s_ana:
            ...
            if df_rows.empty:
                ...
                df_rows = pd.DataFrame(columns=["domain_code","stat_code","label","demand","availability","feasibility"])
...
with tab_s_gen:
    st.markdown("### " + t(lang, "Génération du classeur d'analyse", "Generate the analysis workbook"))
    st.write(
        t(
            lang,
            "Cet onglet génère un fichier Excel d’analyse à partir des réponses collectées, en s’appuyant sur un classeur modèle (template).",
            "This tab generates an Excel analysis workbook from collected responses, using a template workbook."
        )
    )

    tpl = find_analysis_template_path()
    if tpl:
        st.success(t(lang, "Classeur modèle détecté :", "Template workbook found:") + f" {tpl}")
    else:
        st.error(
            t(
                lang,
                "Classeur modèle introuvable. Ajoutez le fichier 'Classeur_analyse_consultation_STATAFRIC_final.xlsx' à la racine ou au dossier 'data/', ou définissez ANALYSIS_TEMPLATE_XLSX.",
                "Template workbook not found. Add 'Classeur_analyse_consultation_STATAFRIC_final.xlsx' at repo root or under 'data/', or set ANALYSIS_TEMPLATE_XLSX."
            )
        )

    if st.button(t(lang, "Générer maintenant", "Generate now"), key="gen_analysis_wb_btn", disabled=(tpl is None)):
        try:
            with st.spinner(t(lang, "Génération en cours…", "Generating…")):
                df_long_gen = load_longlist()
                wb_bytes, wb_name, wb_info = build_analysis_workbook_bytes(df, payloads, df_long=df_long_gen)

            st.session_state["analysis_wb_bytes"] = wb_bytes
            st.session_state["analysis_wb_name"] = wb_name
            st.session_state["analysis_wb_info"] = wb_info
            st.session_state["analysis_wb_generated_at"] = datetime.now(timezone.utc).isoformat(timespec="seconds")

            st.success(t(lang, "Classeur généré avec succès.", "Workbook generated successfully."))
            st.caption(wb_info)
        except Exception as e:
            st.error(t(lang, "Échec de génération :", "Generation failed:") + f" {e}")

with tab_s_dl:
    st.markdown("### " + t(lang, "Téléchargement du classeur", "Download workbook"))
    wb_bytes = st.session_state.get("analysis_wb_bytes", None)
    wb_name = st.session_state.get("analysis_wb_name", "Classeur_analyse_consultation_STATAFRIC.xlsx")
    wb_info = st.session_state.get("analysis_wb_info", "")
    wb_ts = st.session_state.get("analysis_wb_generated_at", "")

    if not wb_bytes:
        st.info(
            t(
                lang,
                "Aucun classeur n’a encore été généré. Allez d’abord dans l’onglet « Générer le classeur d’analyse ».",
                "No workbook has been generated yet. First go to the “Generate analysis workbook” tab."
            )
        )
    else:
        st.download_button(
            label=t(lang, "Télécharger le classeur (.xlsx)", "Download workbook (.xlsx)"),
            data=wb_bytes,
            file_name=wb_name,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="dl_analysis_wb_btn"
        )
        if wb_ts:
            st.caption(t(lang, "Généré le", "Generated on") + f" : {wb_ts}")
        if wb_info:
            st.caption(wb_info)
    with tab_sec:
