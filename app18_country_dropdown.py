...
# Remarque : 0 = NSP / DK (ne sait pas) ; 1–3 = intensité croissante selon le libellé.
SCORE_SCALES = {
    "demand": {
...
def countries_catalog() -> Tuple[List[str], Dict[str, str], Dict[str, str]]:
    ...
    Source : COUNTRY_ISO3_with_EN.xlsx (recommandé), tolère aussi :
    - COUNTRY_ISO3.xlsx (sans colonne EN)
    - fichiers placés à la racine du dépôt ou dans /data.
    """
    ...
    iso_col = _col("COUNTRY_ISO3")
    fr_col = _col("COUNTRY_NAME_FR")
    en_col = _col("COUNTRY_NAME_EN") if "COUNTRY_NAME_EN" in cols else None
    ...
    for _, r in df.iterrows():
        ...
        if not iso3 or iso3.lower() == "nan":
            continue
        fr = str(r.get(fr_col, "")).strip()
        en = str(r.get(en_col, "")).strip() if en_col else ""
        iso3_to_fr[iso3] = fr
        iso3_to_en[iso3] = en
...
def rubric_2(lang: str) -> None:
    ...
    with col1:
        ...
        # Valeur courante (tolère "ISO3 | Nom" ou texte libre)
        ...
        # Options triées selon la langue (FR/EN)
