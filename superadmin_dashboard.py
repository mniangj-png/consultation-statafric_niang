...
def main() -> None:
    ...
    col1.metric("Soumissions finales", len(df))
    ...
    tab1, tab2, tab3 = st.tabs(["Classeur - aperçu", "Rapport - contenu synthétique", "Paramètres"])  # st.tabs
    with tab1:
    ...
    with tab2:
        st.markdown(
            "Le rapport DOCX généré comprend une vue d’ensemble, la distribution des validations globales, la position finale des institutions, les points méthodologiques les plus réservés, les domaines les plus réservés et un extrait des justifications."
        )
    ...
    with tab3:
        st.code(
            "\n".join(
                [
                    f"GitHub owner : {get_github_config_from_streamlit(st).owner}",
                    f"GitHub repo  : {get_github_config_from_streamlit(st).repo}",
                    f"GitHub branch: {get_github_config_from_streamlit(st).branch}",
                ]
            )
        )
