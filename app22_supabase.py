...
def db_save_submission(submission_id: str, lang: str, email: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Enregistre une soumission avec JSON obligatoire dans Supabase Storage.

    Stratégie (pour cohérence Storage ↔ DB) :
    1) Génère l'ID (déjà fourni) et upload d'abord le fichier JSON dans Storage.
    2) Insère ensuite la ligne dans Postgres (jsonb).
       - Si l'insertion échoue, on supprime le fichier Storage (best-effort).
    """
    if not _sb_enabled():
        raise RuntimeError("Supabase non configuré (SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).")
    ...
    if not SUPABASE_STORAGE_BUCKET:
        raise RuntimeError(
            "Storage obligatoire : SUPABASE_STORAGE_BUCKET est vide. "
            "Créez un bucket Supabase Storage puis définissez SUPABASE_STORAGE_BUCKET."
        )
    ...
    try:
        ...
        sb.storage.from_(SUPABASE_STORAGE_BUCKET).upload(
            ...
            b,
            file_options={
                # Important : "content-type" en minuscules (bug connu côté Storage avec "Content-Type")
                "content-type": "application/json",
                # overwrite si le path existe déjà
                "x-upsert": "true",
            },
        )
    except Exception as e:
        raise RuntimeError(f"Échec d'upload Storage ({SUPABASE_STORAGE_BUCKET}/{storage_path}) : {e}")

    # 2) Insertion Postgres (payload jsonb)
    payload_col_primary = SUPABASE_PAYLOAD_COL or "payload"
    ...
    try:
        try:
            resp = sb.table(SUPABASE_SUBMISSIONS_TABLE).insert(row).execute()
            _sb_exec(resp)
            ...
            if (payload_col_primary in msg) and ("PGRST" in msg or "Could not find" in msg or "column" in msg or "schema" in msg):
                ...
                resp = sb.table(SUPABASE_SUBMISSIONS_TABLE).insert(row2).execute()
                _sb_exec(resp)
                used_payload_col = payload_col_fallback
            else:
                raise
    except Exception as e:
        # rollback best-effort du fichier Storage pour éviter un JSON "orphelin"
        try:
    ...
    # Stockage fichier JSON : attention au header "content-type" en minuscules (cf. bug connu).
    storage_path = f"submissions/{submission_id}.json"
    try:
        b = json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8")
        sb.storage.from_(SUPABASE_STORAGE_BUCKET).upload(
        ...
        info.update({"storage_ok": True, "storage_path": storage_path})
    except Exception as e:
        info.update({"storage_ok": False, "storage_path": storage_path, "storage_error": str(e)})
    return info
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        # Enregistrement Supabase (Postgres jsonb) + fichier JSON (Storage obligatoire)
        try:
        ...
        # Informations (sans exposer de secrets)
        try:
            if isinstance(sb_info, dict):
                payload_col = sb_info.get("payload_col")
                if payload_col:
                    st.caption(t(lang, f"JSON : stocké dans Postgres (colonne : {payload_col}).",
                                 f"JSON: stored in Postgres (column: {payload_col})."))
                else:
                    st.caption(t(lang, "JSON : stocké dans Postgres.", "JSON: stored in Postgres."))

                st.caption(t(lang, "JSON : fichier enregistré dans Supabase Storage (obligatoire).",
                             "JSON: file saved in Supabase Storage (required)."))
        except Exception:
            pass
