...
SUPABASE_STORAGE_BUCKET = (os.getenv("SUPABASE_STORAGE_BUCKET", "") or "").strip()
SUPABASE_PAYLOAD_COL = (os.getenv("SUPABASE_PAYLOAD_COL", "payload") or "payload").strip()
SUPABASE_DRAFT_PAYLOAD_COL = (os.getenv("SUPABASE_DRAFT_PAYLOAD_COL", "payload") or "payload").strip()
# Si votre schéma utilise une autre colonne (ex. payload_json), définissez SUPABASE_PAYLOAD_COL=payload_json.


def _sb_enabled() -> bool:
    return bool((os.getenv("SUPABASE_URL") or "").strip()) and bool((os.getenv("SUPABASE_SERVICE_ROLE_KEY") or "").strip())
...
def _sb_client() -> Client:
    ...
    if not url or not key:
        raise RuntimeError("Supabase non configuré (SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).")
...
def _maybe_purge_old_data() -> None:
    """Purge best-effort des données anciennes (45–60 jours).

    - Base Postgres : suppression par date
    - Storage (optionnel) : suppression des JSON correspondants (best-effort)
    """
    if not _sb_enabled():
        return
    ...
    # 1) Pré-lister les IDs à supprimer dans Storage (avant suppression DB)
    ...
    if bucket:
        try:
            ...
            paths = [f"submissions/{r.get('submission_id')}.json" for r in old if r.get("submission_id")]
        except Exception:
            paths = []
...
def db_save_submission(submission_id: str, lang: str, email: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Enregistre une soumission dans Supabase.

    Exigences :
    - Table Postgres : SUPABASE_SUBMISSIONS_TABLE (défaut : submissions)
    - Colonne JSONB : payload (ou payload_json)
    - Storage : fichier JSON obligatoire si SUPABASE_STORAGE_REQUIRED=1 (défaut).

    Stratégie (cohérence) :
    1) Upload Storage (si activé / requis)
    2) Insert DB
    3) Vérifications (relecture DB + téléchargement Storage)
    4) Rollback best-effort si incohérence
    """
    if not _sb_enabled():
        raise RuntimeError("Supabase non configuré (SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).")
    ...
    # Lecture dynamique (secrets Streamlit Cloud + env)
    bucket = ((_get_secret_or_env("SUPABASE_STORAGE_BUCKET")[0]) or SUPABASE_STORAGE_BUCKET or "").strip()
    table = ((_get_secret_or_env("SUPABASE_SUBMISSIONS_TABLE")[0]) or SUPABASE_SUBMISSIONS_TABLE or "submissions").strip() or "submissions"
    payload_col_primary = (((_get_secret_or_env("SUPABASE_PAYLOAD_COL")[0]) or SUPABASE_PAYLOAD_COL or "payload")).strip() or "payload"
    payload_col_fallback = "payload_json" if payload_col_primary == "payload" else "payload"

    storage_required_raw = ((_get_secret_or_env("SUPABASE_STORAGE_REQUIRED")[0]) or os.getenv("SUPABASE_STORAGE_REQUIRED", "1") or "1").strip().lower()
    storage_required = storage_required_raw in ("1", "true", "yes", "y", "on")

    # Préparer la ligne DB
    row_base = {
    ...
    storage_path = f"submissions/{submission_id}.json"
    uploaded_ok = False

    # 1) Storage (si requis / si bucket défini)
    if not bucket and storage_required:
        raise RuntimeError("Storage obligatoire : SUPABASE_STORAGE_BUCKET est vide ou non défini (secrets / env).")
    ...
    if bucket:
        try:
            ...
            if storage_required:
                raise RuntimeError(f"Storage obligatoire : upload impossible vers bucket '{bucket}' (chemin : {storage_path}).") from e

    # 2) Insert DB
    used_payload_col = payload_col_primary
    try:
        row = dict(row_base)
        row[payload_col_primary] = payload
        resp = sb.table(table).insert(row).execute()
        _sb_exec(resp)
    except Exception as e:
        msg = str(e)

        # Doublon (email déjà soumis) : message explicite côté utilisateur
        if ("duplicate key value" in msg.lower()) or ("23505" in msg) or ("unique constraint" in msg.lower()):
            # rollback storage si on avait upload
            if bucket and uploaded_ok:
                try:
        ...
        if (payload_col_primary in msg) and ("PGRST" in msg or "Could not find" in msg or "column" in msg or "schema" in msg):
            row2 = dict(row_base)
            row2[payload_col_fallback] = payload
            resp2 = sb.table(table).insert(row2).execute()
            _sb_exec(resp2)
            used_payload_col = payload_col_fallback
        else:
            # rollback storage si on avait upload
            if bucket and uploaded_ok:
                try:
    ...
    # 3) Vérification DB (relecture)
    try:
        chk = sb.table(table).select("submission_id").eq("submission_id", submission_id).limit(1).execute()
        chk_data = _sb_exec(chk) or []
    except Exception as e:
        # rollback best-effort (DB + Storage)
        try:
            sb.table(table).delete().eq("submission_id", submission_id).execute()
        except Exception:
            pass
        if bucket and uploaded_ok:
            try:
                ...
                pass
        raise RuntimeError("Vérification DB impossible après insertion (connexion / API).") from e
    ...
    if not chk_data:
        ...
        if bucket and uploaded_ok:
            try:
                ...
                pass
        raise RuntimeError("Incohérence : insertion annoncée mais soumission introuvable dans la table (vérification).")

    # 4) Vérification Storage (si bucket)
    if bucket:
        try:
            _ = sb.storage.from_(bucket).download(storage_path)
        except Exception as e:
            # Si storage est obligatoire : rollback DB pour rester cohérent
            if storage_required:
                try:
                    sb.table(table).delete().eq("submission_id", submission_id).execute()
                except Exception:
                    pass
                raise RuntimeError(f"Storage obligatoire : fichier JSON introuvable ou illisible après upload (bucket '{bucket}', chemin '{storage_path}').") from e

    return {
        "db_ok": True,
        "payload_col": used_payload_col,
        "table": table,
        "storage_required": storage_required,
        "storage_enabled": bool(bucket),
        "storage_ok": bool(bucket and uploaded_ok),
        "bucket": bucket,
        "storage_path": storage_path if bucket else None,
    }
...
def db_save_draft(draft_id: str, email: str, payload: Dict[str, Any]) -> None:
    ...
    if not draft_id or not email:
        return
    payload_col_primary = SUPABASE_DRAFT_PAYLOAD_COL or "payload"
    payload_col_fallback = "payload_json" if payload_col_primary == "payload" else "payload"
    row_base = {
        "draft_id": draft_id,
    ...
    try:
        ...
        if (payload_col_primary in msg) and ("PGRST" in msg or "Could not find" in msg or "column" in msg or "schema" in msg):
            row2 = dict(row_base)
            row2[payload_col_fallback] = payload
            _sb_exec(sb.table(SUPABASE_DRAFTS_TABLE).upsert(row2).execute())
        else:
            raise
def db_load_draft(draft_id: str) -> Optional[Dict[str, Any]]:
    if not _sb_enabled():
...
def db_read_submissions(limit: int = 2000) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame(columns=["submission_id", "submitted_at_utc", "lang", "email", "payload_json"])
    # Compat : admin_dashboard attend une string JSON
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        # Enregistrement Supabase (DB + Storage obligatoire par défaut)
        try:
        ...
        payload_col = (sb_info or {}).get("payload_col", "payload")
        table = (sb_info or {}).get("table", SUPABASE_SUBMISSIONS_TABLE)
        bucket = (sb_info or {}).get("bucket", "")
        storage_path = (sb_info or {}).get("storage_path", "")

        st.caption(t(lang, f"JSON : stocké dans Postgres (table : {table}, colonne : {payload_col}).",
                     f"JSON: stored in Postgres (table: {table}, column: {payload_col})."))

        if bucket:
            st.caption(t(lang, f"JSON : fichier Storage enregistré (bucket : {bucket}, chemin : {storage_path}).",
                         f"JSON: Storage file saved (bucket: {bucket}, path: {storage_path})."))
        else:
            st.warning(t(lang, "Storage non activé : SUPABASE_STORAGE_BUCKET est vide. "
                               "Avec Storage obligatoire, l’envoi aurait dû échouer ; vérifiez vos secrets.",
                         "Storage not enabled: SUPABASE_STORAGE_BUCKET is empty. "
                         "With required Storage, submission should have failed; check your secrets."))
