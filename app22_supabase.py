...
# Dépendances optionnelles (non requises en production Supabase)
try:
...
# =========================
# Stockage Supabase (Postgres uniquement : réponses JSON)
# =========================

# Objectif : enregistrer les réponses dans Postgres (table "submission_json" par défaut),
# dans une colonne JSON/JSONB (payload_json par défaut), SANS recours à Supabase Storage.
#
# Personnalisation (secrets Streamlit Cloud ou variables d'environnement) :
# - SUPABASE_URL
# - SUPABASE_SERVICE_ROLE_KEY (recommandé côté serveur) OU SUPABASE_ANON_KEY (si RLS configuré)
# - SUPABASE_SUBMISSIONS_TABLE      (défaut : submission_json)
# - SUPABASE_PAYLOAD_COL            (défaut : payload_json)  # ex. payload, submission_json
# - SUPABASE_EMAIL_COL              (défaut : email)         # optionnel, utile pour dédoublonnage
# - SUPABASE_SUBMISSION_ID_COL      (défaut : submission_id) # optionnel
# - SUPABASE_SUBMITTED_AT_COL       (défaut : submitted_at_utc) # optionnel (sinon created_at)
# - SUPABASE_LANG_COL               (défaut : lang)          # optionnel
#
# Tables annexes (si vous utilisez le mode "brouillon" et la configuration) :
# - SUPABASE_DRAFTS_TABLE           (défaut : drafts)
# - SUPABASE_DRAFT_PAYLOAD_COL      (défaut : payload_json)
# - SUPABASE_CONFIG_TABLE           (défaut : app_config)
#
# Rétention (optionnel) :
# - DATA_RETENTION_DAYS             (défaut : 60)


SUPABASE_SUBMISSIONS_TABLE = os.getenv("SUPABASE_SUBMISSIONS_TABLE", "submission_json").strip() or "submission_json"
SUPABASE_DRAFTS_TABLE = os.getenv("SUPABASE_DRAFTS_TABLE", "drafts").strip() or "drafts"
SUPABASE_CONFIG_TABLE = os.getenv("SUPABASE_CONFIG_TABLE", "app_config").strip() or "app_config"

SUPABASE_PAYLOAD_COL = (os.getenv("SUPABASE_PAYLOAD_COL", "payload_json") or "payload_json").strip() or "payload_json"
SUPABASE_DRAFT_PAYLOAD_COL = (os.getenv("SUPABASE_DRAFT_PAYLOAD_COL", "payload_json") or "payload_json").strip() or "payload_json"

SUPABASE_EMAIL_COL = (os.getenv("SUPABASE_EMAIL_COL", "email") or "email").strip() or "email"
SUPABASE_SUBMISSION_ID_COL = (os.getenv("SUPABASE_SUBMISSION_ID_COL", "submission_id") or "submission_id").strip() or "submission_id"
SUPABASE_SUBMITTED_AT_COL = (os.getenv("SUPABASE_SUBMITTED_AT_COL", "submitted_at_utc") or "submitted_at_utc").strip() or "submitted_at_utc"
SUPABASE_LANG_COL = (os.getenv("SUPABASE_LANG_COL", "lang") or "lang").strip() or "lang"


def _sb_key() -> str:
    # Priorité : secrets Streamlit Cloud, puis env
    k = (st.secrets.get("SUPABASE_SERVICE_ROLE_KEY", "") if hasattr(st, "secrets") else "") or ""
    if not k:
        k = (st.secrets.get("SUPABASE_ANON_KEY", "") if hasattr(st, "secrets") else "") or ""
    if not k:
        k = (os.getenv("SUPABASE_SERVICE_ROLE_KEY") or "").strip()
    if not k:
        k = (os.getenv("SUPABASE_ANON_KEY") or "").strip()
    if not k:
...
def _sb_enabled() -> bool:
    return bool((os.getenv("SUPABASE_URL") or "").strip()) and bool(_sb_key())

def _sb_submissions_table_candidates() -> List[str]:
    """Liste des noms de tables possibles pour les submissions.

    Permet la compatibilité entre : submission_json et submission-json.
    Priorité : SUPABASE_SUBMISSIONS_TABLE (secrets/env) puis variantes.
    """
    t_secret, _src = _get_secret_or_env("SUPABASE_SUBMISSIONS_TABLE")
    base = (t_secret or SUPABASE_SUBMISSIONS_TABLE or "").strip()
    cands: List[str] = []
    ...
    # Valeurs de secours
    _add("submission_json")
    _add("submission-json")
    return cands
...
def _sb_client() -> Client:
    ...
    if not url or not key:
        raise RuntimeError("Supabase non configuré (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY ou SUPABASE_ANON_KEY).")
    return create_client(url, key)


def _sb_exec(resp: Any) -> Any:
    # supabase-py renvoie un objet avec .data / .error
...
def _is_schema_error(msg: str) -> bool:
    m = (msg or "").lower()
    return ("pgrst" in m) or ("could not find" in m) or ("column" in m) or ("schema" in m)


def _is_duplicate_error(msg: str) -> bool:
    m = (msg or "").lower()
    return ("duplicate key value" in m) or ("23505" in m) or ("unique constraint" in m)



def _maybe_purge_old_data() -> None:
    """Purge best-effort des données anciennes (45–60 jours) dans Postgres uniquement."""
    if not _sb_enabled():
        return

    # fréquence : toutes les 12 heures max (par instance)
    last = float(st.session_state.get("_last_purge_ts", 0.0) or 0.0)
    if time.time() - last < 12 * 3600:
        return
    st.session_state["_last_purge_ts"] = time.time()

    try:
        days = int(os.getenv("DATA_RETENTION_DAYS", "60"))
    except Exception:
        days = 60
    days = max(1, min(days, 365))
    cutoff_iso = (datetime.now(timezone.utc) - pd.Timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ")

    sb = _sb_client()

    # Submissions : suppression best-effort selon la date (submitted_at_utc de préférence)
    for table in _sb_submissions_table_candidates():
        try:
            _sb_exec(
                sb.table(table)
                  .delete()
                  .lt("submitted_at_utc", cutoff_iso)
                  .execute()
            )
            break
        except Exception as e:
    ...
    # Drafts : updated_at_utc, sinon created_at
    try:
        _sb_exec(
            sb.table(SUPABASE_DRAFTS_TABLE)
              .delete()
              .lt("updated_at_utc", cutoff_iso)
              .execute()
        )
    except Exception as e:
        if _is_schema_error(str(e)):
...
def db_get_config(k: str) -> Optional[str]:
    ...
    if not k:
        return None
    sb = _sb_client()
    data = _sb_exec(sb.table(SUPABASE_CONFIG_TABLE).select("v").eq("k", k).limit(1).execute()) or []
    if not data:
        return None
    return data[0].get("v")
...
def db_set_config(k: str, v: str) -> None:
    if not _sb_enabled():
        raise RuntimeError("Supabase non configuré (SUPABASE_URL + clé).")
    k = (k or "").strip()
    if not k:
        return
    sb = _sb_client()
    _sb_exec(sb.table(SUPABASE_CONFIG_TABLE).upsert({"k": k, "v": str(v)}).execute())
...
def db_delete_config(k: str) -> None:
    ...
    if not k:
        return
    sb = _sb_client()
    _sb_exec(sb.table(SUPABASE_CONFIG_TABLE).delete().eq("k", k).execute())



def db_email_exists(email: str) -> bool:
    """Vérifie si un email a déjà soumis une réponse (best-effort).

    Retourne False si Supabase n'est pas configuré, ou si la table/colonne n'existe pas.
    """
    if not _sb_enabled():
        return False
    email = (email or "").strip().lower()
...
def db_save_submission(submission_id: str, lang: str, email: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Enregistre une soumission dans Supabase (Postgres), table `submission_json` (ou `submission-json`).

    Schéma attendu (exact) :
      - submission_id (uuid)
    ...
    _maybe_purge_old_data()

    sb = _sb_client()
    email = (email or "").strip().lower()
    submitted_at = now_utc_iso()

    # Sécurise un minimum le payload
    payload = payload or {}
    payload.setdefault("submission_id", submission_id)
    payload.setdefault("submitted_at_utc", submitted_at)
    ...
    last_schema_err: Optional[Exception] = None
    for table in _sb_submissions_table_candidates():
        try:
            resp = sb.table(table).insert(row).execute()
            inserted = _sb_exec(resp)
            return {
                "table": table,
                "used_row_keys": list(row.keys()),
                "inserted": (inserted[0] if isinstance(inserted, list) and inserted else inserted),
            }
        except Exception as e:
            msg = str(e)
            if _is_duplicate_error(msg):
                raise RuntimeError("Doublon détecté : ce questionnaire a déjà été envoyé avec cet email.") from e
    ...
    raise RuntimeError(
        "Insertion impossible dans Supabase. Vérifiez le nom exact de la table "
        "(SUPABASE_SUBMISSIONS_TABLE) et le schéma attendu : submission_id, submitted_at_utc, lang, email, payload_json. "
        f"Détail : {last_schema_err}"
    ) from last_schema_err
...
def db_save_draft(draft_id: str, email: str, payload: Dict[str, Any]) -> None:
    ...
    payload.setdefault("updated_at_utc", now_utc_iso())
    payload.setdefault("email", email)

    payload_col = (((_get_secret_or_env("SUPABASE_DRAFT_PAYLOAD_COL")[0]) or SUPABASE_DRAFT_PAYLOAD_COL or "payload_json")).strip() or "payload_json"

    row_variants = [
        {"draft_id": draft_id, "updated_at_utc": now_utc_iso(), "email": email, payload_col: payload},
        {"draft_id": draft_id, "email": email, payload_col: payload},
        {"draft_id": draft_id, payload_col: payload},
        {"draft_id": draft_id, "payload_json": payload},
    ...
    for row in row_variants:
        try:
            _sb_exec(sb.table(SUPABASE_DRAFTS_TABLE).upsert(row).execute())
            return
        except Exception as e:
            if _is_schema_error(str(e)):
                last_schema_err = e
    ...
    # best-effort : ne pas bloquer l'utilisateur sur l'autosave
    if last_schema_err:
        return
...
def db_load_draft(draft_id: str) -> Optional[Dict[str, Any]]:
    ...
    payload_col = (((_get_secret_or_env("SUPABASE_DRAFT_PAYLOAD_COL")[0]) or SUPABASE_DRAFT_PAYLOAD_COL or "payload_json")).strip() or "payload_json"
    col_candidates = [payload_col, "payload_json", "payload"]
...
def db_delete_draft(draft_id: str) -> None:
    ...
    if not draft_id:
        return
    _sb_exec(sb.table(SUPABASE_DRAFTS_TABLE).delete().eq("draft_id", draft_id).execute())



def db_read_submissions(limit: int = 2000) -> pd.DataFrame:
    """Lecture des submissions (Supabase), normalisée vers :
    ...
    for table in _sb_submissions_table_candidates():
        try:
            resp = (
                sb.table(table)
                  .select("submission_id, submitted_at_utc, lang, email, payload_json")
                  .order("submitted_at_utc", desc=True)
                  .limit(limit)
                  .execute()
            )
            data = _sb_exec(resp) or []
            df = pd.DataFrame(data)
            if df.empty:
                return pd.DataFrame(columns=["submission_id", "submitted_at_utc", "lang", "email", "payload_json"])

            df["payload_json"] = df["payload_json"].apply(
                lambda x: json.dumps(x, ensure_ascii=False) if isinstance(x, (dict, list)) else (x or "")
            )
            return df[["submission_id", "submitted_at_utc", "lang", "email", "payload_json"]]
        except Exception as e:
            if _is_schema_error(str(e)):
                continue
            raise
...
def db_dump_csv_bytes(limit: int = 2000000) -> bytes:
    df = db_read_submissions(limit=min(limit, 50000))
    return df.to_csv(index=False).encode("utf-8-sig")
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    # Pré-contrôle email (best-effort) : ne pas bloquer l'interface si Supabase est momentanément indisponible.
    already_in_db = False
    if email:
        try:
            ...
            st.warning(
                t(
                    lang,
                    "Impossible de vérifier pour l’instant si cet email a déjà répondu (connexion Supabase). "
                    "Vous pouvez continuer ; en cas de doublon, l’envoi sera refusé au moment de l’enregistrement.",
                    "Cannot check right now whether this email already submitted (Supabase connectivity). "
                    "You can continue; duplicates will be rejected during saving."
                )
            )
            already_in_db = False
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        # Enregistrement Supabase (Postgres uniquement : JSON)
        try:
            sb_info = db_save_submission(submission_id, lang, email, payload)
        except Exception as e:
            st.error(t(lang, "Échec d'enregistrement Supabase : ", "Supabase save failed: ") + str(e))
            ...
            st.info(t(lang, "Conservez ce fichier. Vous pourrez le transmettre pour intégration ultérieure.",
                      "Keep this file. You can share it later for integration."))
            return
        # Informations (best-effort)
        payload_col = (sb_info or {}).get("payload_col", "payload_json")
        table = (sb_info or {}).get("table", SUPABASE_SUBMISSIONS_TABLE)
        used_keys = (sb_info or {}).get("used_row_keys", [])

        st.caption(t(lang, f"JSON : enregistré dans Postgres (table : {table}, colonne : {payload_col}).",
                     f"JSON: saved in Postgres (table: {table}, column: {payload_col})."))
        if used_keys:
            st.caption(t(lang, f"Champs insérés : {', '.join([str(x) for x in used_keys])}.",
                         f"Inserted fields: {', '.join([str(x) for x in used_keys])}."))
