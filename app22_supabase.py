...
st.set_page_config(page_title="Consultation STATAFRIC", layout="centered", initial_sidebar_state="collapsed")

# --- STABILISATION AFFICHAGE (évite les variations de largeur liées à la scrollbar) ---
st.markdown("""
<style>
...
# =========================
# Supabase (Postgres + Storage) : configuration et helpers
# =========================

def _get_cfg(key: str, default: str = "") -> str:
    """Lit une configuration depuis (1) env vars, (2) secrets top-level, (3) secrets connections.supabase.
    Compatible Streamlit Cloud + Render.
    """
    v = os.getenv(key)
    ...
    # secrets connections.supabase
    try:
        sup = st.secrets.get("connections", {}).get("supabase", {})
        if key in sup:
            return str(sup.get(key, default)).strip()
        lk = key.lower()
        if lk in sup:
            return str(sup.get(lk, default)).strip()
        if key == "SUPABASE_URL" and "url" in sup:
            return str(sup.get("url", default)).strip()
        if key == "SUPABASE_SERVICE_ROLE_KEY" and "key" in sup:
            return str(sup.get("key", default)).strip()
        if key == "SUPABASE_STORAGE_BUCKET" and "bucket" in sup:
            return str(sup.get("bucket", default)).strip()
    except Exception:
        pass
...
def _get_supabase_client() -> Client:
    url = _normalize_supabase_url(_get_cfg("SUPABASE_URL"))
    key = _get_cfg("SUPABASE_SERVICE_ROLE_KEY")
...
def _supabase_diag() -> Dict[str, str]:
    url = _normalize_supabase_url(_get_cfg("SUPABASE_URL"))
    host = urlparse(url).netloc if url else ""
    bucket = _get_cfg("SUPABASE_STORAGE_BUCKET")
    return {"url_base": url, "host": host, "bucket": bucket}

def offer_local_json_backup(lang: str, submission_id: str, payload: Dict[str, Any], reason: str = "") -> None:
    diag = _supabase_diag()
    st.error(t(
        lang,
        "Impossible d’enregistrer en ligne pour le moment. Votre saisie n’est pas perdue : téléchargez la sauvegarde JSON ci-dessous, puis réessayez plus tard.",
        "Online saving is temporarily unavailable. Your input is not lost: download the JSON backup below, then try again later."
    ))
    if reason:
        st.caption(t(lang, "Détail technique (résumé) : ", "Technical detail (summary): ") + reason)

    json_str = json.dumps(payload, ensure_ascii=False, indent=2)
    st.download_button(
        label=t(lang, "⬇️ Télécharger la sauvegarde JSON", "⬇️ Download JSON backup"),
        data=json_str.encode("utf-8"),
        file_name=f"app22_submission_{submission_id}.json",
        mime="application/json",
        use_container_width=True,
    )

    st.text_area(
        t(lang, "Copie de secours (JSON)", "Backup copy (JSON)"),
        value=json_str,
        height=260,
    )
    ...
    st.info(
        t(
            lang,
            "Conseil : conservez ce fichier. Vous pourrez le transmettre à l’équipe technique si besoin, ou réessayer l’envoi quand la connexion sera rétablie.",
            "Tip: keep this file. You can share it with the technical team if needed, or retry submission once connectivity is restored.",
        )
    )
    ...
    st.caption(
        ...
        + f"url={diag.get('url_base','')} ; host={diag.get('host','')} ; bucket={diag.get('bucket','')}"
    )

def _storage_required_bucket(lang: str) -> str:
    bucket = _get_cfg("SUPABASE_STORAGE_BUCKET")
    if not bucket:
        st.warning(t(
            lang,
            "Storage obligatoire : SUPABASE_STORAGE_BUCKET n’est pas défini. L’envoi en ligne échouera ; utilisez la sauvegarde JSON.",
            "Storage is mandatory: SUPABASE_STORAGE_BUCKET is not set. Online submission will fail; use the JSON backup."
        ))
    return bucket
...
# =========================
# Barèmes de notation (multicritères)
# =========================
# Convention : NS = 0 (non renseigné / ne sait pas)
SCORE_SCALES = {
    "demand": {
        "fr": {0: "NS", 1: "Faible", 2: "Moyenne", 3: "Forte"},
        "en": {0: "NS", 1: "Low", 2: "Medium", 3: "High"},
    },
    "availability": {
        "fr": {0: "NS", 1: "Non disponible", 2: "Partiellement disponible", 3: "Disponible"},
        "en": {0: "NS", 1: "Not available", 2: "Partly available", 3: "Available"},
    },
    "feasibility": {
        "fr": {0: "NS", 1: "Faible", 2: "Moyenne", 3: "Élevée"},
        "en": {0: "NS", 1: "Low", 2: "Medium", 3: "High"},
    },
}
...
def rubric_send(lang: str, df_long: pd.DataFrame) -> None:
    ...
    if st.session_state.get("_supabase_unreachable"):
        diag = _supabase_diag()
        st.warning(t(
            lang,
            "Connexion Supabase instable ou indisponible : le contrôle d’unicité email n’a pas pu être vérifié. Vous pouvez quand même tenter l’envoi ; en cas d’échec, téléchargez la sauvegarde JSON.",
            "Supabase connectivity is unstable or unavailable: the email uniqueness check could not be verified. You may still try to submit; if it fails, download the JSON backup."
        ))
        st.caption(t(lang, "Diagnostic : ", "Diagnostics: ") + f"host={diag.get('host','')} ; bucket={diag.get('bucket','')}")
    ...
    if st.button(t(lang, "✅ ENVOYER et enregistrer", "✅ SUBMIT and save"), disabled=disable_submit):
        ...
        # Enregistrement : Storage obligatoire (JSON) + Postgres
        try:
            sb_info = db_save_submission(submission_id, lang, email, payload)
        except Exception as e:
            offer_local_json_backup(lang, submission_id, payload, reason=str(e))
            st.stop()

        # Confirmation (sans secrets)
        try:
            if isinstance(sb_info, dict):
                bucket = sb_info.get("storage_bucket")
                spath = sb_info.get("storage_path")
                if spath:
                    st.caption(t(lang, f"JSON : fichier enregistré dans Supabase Storage ({bucket}:{spath}).",
                                 f"JSON: file saved in Supabase Storage ({bucket}:{spath})."))
            st.caption(t(lang, "JSON : stocké dans Postgres (colonne : payload).", "JSON: stored in Postgres (column: payload)."))
        except Exception:
            pass
...
def main() -> None:
    # (set_page_config déjà défini plus haut)
